...
(async () => {
  ...
  try {
    ...
    log('Has app content', bodyText.includes('TransObra') || bodyText.includes('Dashboard') || bodyText.includes('Contratos') || bodyText.includes('Perfil'));
    log('Sidebar visible', await page.locator('nav, [class*="sidebar"], [class*="Sidebar"]').first().isVisible().catch(() => false));

    // ===== TEST 6: Navigate Contratos =====
    console.log('\n=== TEST 6: Contratos ===');
    await page.goto(URL + '/contratos', { waitUntil: 'networkidle', timeout: 15000 });
    await page.waitForTimeout(2000);
    let text = await page.textContent('body');
    log('Contratos page loaded', text.includes('Contrato'));
    log('Has contract data (CT-)', text.includes('CT-'));

    // ===== TEST 7: Novo Contrato modal =====
    console.log('\n=== TEST 7: Novo Contrato modal ===');
    const novoBtn = await page.locator('button:has-text("Novo Contrato")').first();
    log('Novo Contrato button visible', await novoBtn.isVisible().catch(() => false));
    await novoBtn.click();
    ...
    // ===== TEST 8: Comprovantes =====
    console.log('\n=== TEST 8: Comprovantes ===');
    await page.goto(URL + '/comprovantes', { waitUntil: 'networkidle', timeout: 15000 });
    await page.waitForTimeout(2000);
    text = await page.textContent('body');
    log('Comprovantes loaded', text.includes('Comprovante') || text.includes('comprovante'));
    log('Has comprovante data', text.includes('1231') || text.includes('Alpha') || text.includes('CT-'));

    // ===== TEST 9: Assinatura Digital =====
    console.log('\n=== TEST 9: Assinatura Digital ===');
    await page.goto(URL + '/assinatura', { waitUntil: 'networkidle', timeout: 15000 });
    await page.waitForTimeout(2000);
    text = await page.textContent('body');
    log('Assinatura loaded', text.includes('ssinatura'));

    // ===== TEST 10: Perfil =====
    console.log('\n=== TEST 10: Perfil ===');
    await page.goto(URL + '/perfil', { waitUntil: 'networkidle', timeout: 15000 });
    await page.waitForTimeout(2000);
    text = await page.textContent('body');
    log('Perfil loaded', text.includes('erfil') || text.includes('Suporte'));

    // ===== TEST 11: Devolucoes =====
    console.log('\n=== TEST 11: Devolucoes ===');
    await page.goto(URL + '/devolucoes', { waitUntil: 'networkidle', timeout: 15000 });
    await page.waitForTimeout(2000);
    text = await page.textContent('body');
    log('Devolucoes loaded', text.includes('evolu'));

    // ===== TEST 12: Historico =====
    console.log('\n=== TEST 12: Historico ===');
    await page.goto(URL + '/historico', { waitUntil: 'networkidle', timeout: 15000 });
    await page.waitForTimeout(2000);
    text = await page.textContent('body');
    log('Historico loaded', text.includes('istorico') || text.includes('Hist'));

    // ===== TEST 13: Usuarios =====
    console.log('\n=== TEST 13: Usuarios ===');
    await page.goto(URL + '/usuarios', { waitUntil: 'networkidle', timeout: 15000 });
    await page.waitForTimeout(2000);
    text = await page.textContent('body');
    log('Usuarios loaded', text.includes('suario'));

    // ===== TEST 14: Equipamentos =====
    console.log('\n=== TEST 14: Equipamentos ===');
    await page.goto(URL + '/equipamentos', { waitUntil: 'networkidle', timeout: 15000 });
    await page.waitForTimeout(2000);
    text = await page.textContent('body');
    log('Equipamentos loaded', text.includes('quipamento'));

    // ===== TEST 15: Ordens de Servico =====
    console.log('\n=== TEST 15: Ordens de Servico ===');
    await page.goto(URL + '/ordens', { waitUntil: 'networkidle', timeout: 15000 });
    await page.waitForTimeout(2000);
    text = await page.textContent('body');
    log('Ordens loaded', text.includes('rdem') || text.includes('OS'));
