...
export function findContato(lines) {
  const sectionBreak = /^(Refer|Telefone|CNPJ|Bairro|Cidade|Estado|CEP|Endere|N[ºo°]|Data|Hora|Observa|Locat|Atend|Fone|RG|COMPROVANTE)/i;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (m && m[1]) {
      ...
      if (i + 1 < lines.length) {
        ...
        if (next.length > 0 && next.length < 60 && !/^\d/.test(next) && !sectionBreak.test(next)) {
          return next;
        }
      }
    }
...
export function findLocalEntrega(lines) {
  const sectionBreak = /^(DECLAR|DECLARO|Objeto|Qtde\s+Descri|DLoc\s+DDev|LOCADORA|LOCAT[ÁA]RIO|NOME\s+RG|COMPROVANTE\s+DE|ORÇAMENTO|ORCAMENTO|Assinatura|Respons[áa]vel|_\s{5,}|-{5,}|MANAUS,?\s+\d)/i;
  const addressGarbage = /(DECLARO|recebi|objetos?\s+abaixo|discriminad|Qtde\s+Descri|DLoc\s+DDev)/i;

  for (let i = 0; i < lines.length; i++) {
    const m = lines[i].match(FIELD_PATTERNS.localEntrega[0]);
    if (m && m[1]) {
      let v = clean(m[1]);
      if (addressGarbage.test(v)) {
        v = v.replace(addressGarbage, '').replace(/[-–_\s]+$/, '').trim();
      }
      if (v.length > 0) return v;
      ...
      while (j < lines.length) {
        ...
        if (trimmed.length === 0 || sectionBreak.test(trimmed)) break;
        let nextClean = clean(nextLine);
        if (addressGarbage.test(nextClean)) {
          nextClean = nextClean.replace(addressGarbage, '').replace(/[-–_\s]+$/, '').trim();
          if (nextClean.length > 0) v = v + ' ' + nextClean;
          break;
        }
        if (nextClean.length > 0 && nextClean.length < 120) {
          v = v + ' ' + nextClean;
        }
        j++;
      }
...
export function extractAllFields(lines) {
  ...
  if (!fields.local_entrega) {
    const parts = [];
    if (fields.endereco) parts.push(fields.endereco + (fields.numero ? `, ${fields.numero}` : ''));
    if (fields.bairro) parts.push(fields.bairro);
    if (fields.referencia) parts.push(fields.referencia);
    if (fields.cidade) parts.push(fields.cidade + (fields.estado ? `/${fields.estado}` : ''));
    if (fields.cep) parts.push(`CEP: ${fields.cep}`);
    if (parts.length > 0) fields.local_entrega = parts.join(' - ');
  }
