...
function NavLink({ item, location, setMobileOpen }) {
...
function SidebarContent({ navItems, navItemsSec, initials, userName, userEmail, userRole, currentRole, switchRole, handleLogout, setMobileOpen, location }) {
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto py-3">
        <nav className="space-y-0.5">
          {navItems.map(item => <NavLink key={item.path} item={item} location={location} setMobileOpen={setMobileOpen} />)}
        </nav>
        ...
        {navItemsSec.length > 0 && (
          <>
            ...
            <nav className="space-y-0.5">
              {navItemsSec.map(item => <NavLink key={item.path} item={item} location={location} setMobileOpen={setMobileOpen} />)}
            </nav>
          </>
        )}
      </div>
...
export default function Sidebar({ mobileOpen, setMobileOpen }) {
  ...
  const sidebarProps = { navItems, navItemsSec, initials, userName, userEmail, userRole, currentRole, switchRole, handleLogout, setMobileOpen, location };
