    ...
    mutationFn: async (devolucao) => {
      if (isConfigured()) {
        try {
          ...
          localStorage.setItem('devolucoes_local', JSON.stringify(stored));
          return local;
        }
      }
      const local = { id: `DEV-${String(Date.now()).slice(-6)}`, ...devolucao, status: devolucao.status || 'pendente', createdAt: new Date().toISOString() };
    ...
    mutationFn: async ({ id, updates }) => {
      if (isConfigured()) {
        try {
          ...
          if (idx !== -1) { stored[idx] = { ...stored[idx], ...updates }; localStorage.setItem('devolucoes_local', JSON.stringify(stored)); }
          return { id, ...updates };
        }
      }
      const stored = JSON.parse(localStorage.getItem('devolucoes_local') || '[]');
    ...
    mutationFn: async (id) => {
      if (isConfigured()) {
        try {
          ...
          const stored = JSON.parse(localStorage.getItem('devolucoes_local') || '[]');
          localStorage.setItem('devolucoes_local', JSON.stringify(stored.filter((d) => d.id !== id)));
          return id;
        }
      }
      const stored = JSON.parse(localStorage.getItem('devolucoes_local') || '[]');
