...
async function aiExtract(text, tipoDocumento) {
  try {
    ...
    if (!res.ok) {
      console.warn('[AI Extract] Error:', data.error || res.status);
      return null;
    }
    if (data.success && data.data) return data.data;
    return null;
  } catch (err) {
    console.warn('[AI Extract] Network error:', err.message);
    return null;
  }
}
...
function mapAiToFields(ai) {
  ...
  const local = ai.local_entrega || '';
  const builtLocal = !local ? [
    ai.endereco ? `${ai.endereco}${ai.numero ? `, ${ai.numero}` : ''}` : '',
    ai.bairro || '',
    ai.referencia || '',
    ai.cidade ? `${ai.cidade}${ai.estado ? `/${ai.estado}` : ''}` : '',
    ai.cep ? `CEP: ${ai.cep}` : '',
  ].filter(Boolean).join(' - ') : '';
  ...
  return {
    ...
    local_entrega: local || builtLocal,
    ...
    valor_mensal: ai.valor_mensal || 0,
    tipo_documento: ai.tipo_documento || 'entrega',
    condicoes: ai.condicoes || { danificado: false, extraviado: false, testarEmpresa: false },
  };
}
...
function countFilledFields(fields) {
  ...
  delete textFields.equipamentos;
...
export default function PdfImportButton({ onFieldsExtracted }) {
  ...
  const processWithAI = async () => {
    ...
    try {
      ...
      console.warn('[processWithAI] Error:', err.message);
      finishImport(regexResult.parsed, false);
    } finally {
      setProcessing(false);
    }
                ...
                {regexFields.tipo_documento && (
                  <div className={`rounded-lg p-3 border ${
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                      ...
                      {regexFields.endereco && <div><span className="text-gray-500">Endereco:</span> <span className="font-medium">{regexFields.endereco}</span></div>}
                      {regexFields.local_entrega && <div className="col-span-2"><span className="text-gray-500">Local Entrega:</span> <span className="font-medium">{regexFields.local_entrega}</span></div>}
